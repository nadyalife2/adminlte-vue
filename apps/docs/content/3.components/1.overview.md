...
`@colorlib/adminlte-vue` ships a complete set of Vue 3 components for building AdminLTE 4 dashboards. Every
...
```ts
import { LteDashboardLayout, LteCard, LteButton } from '@colorlib/adminlte-vue'
import { LteApexChart, LteDatatable } from '@colorlib/adminlte-vue/plugins'
```

In Nuxt with `@colorlib/adminlte-nuxt`, all of these are auto-registered, so you can use the tags in any
...
app in it. Import from [`@colorlib/adminlte-vue`](/components/layout).
...
Cards, boxes, stats and nav-dropdown widgets. Import from [`@colorlib/adminlte-vue`](/components/widgets).
...
`@colorlib/adminlte-vue`) that turns a menu array into palette commands.

```ts
import { LteCommandPalette, flattenMenuToCommands } from '@colorlib/adminlte-vue'
```

## Form

Form controls, plain and enhanced. Import from [`@colorlib/adminlte-vue`](/components/forms).
...
Interactive utilities, imported from `@colorlib/adminlte-vue` (listed here — no separate detail page).
...
wrap them in `<ClientOnly>`. Import from [`@colorlib/adminlte-vue/plugins`](/components/plugins).
