...
`@colorlib/adminlte-vue` ships the same prebuilt AdminLTE 4 CSS, so your visual result is unchanged — what
...
import { LteDashboardLayout, LteAppContent } from '@colorlib/adminlte-vue'
...
After migrating, the project's correctness gates are `pnpm --filter @colorlib/adminlte-vue type-check` and a
successful demo build:

```bash
pnpm --filter @colorlib/adminlte-vue type-check
pnpm build:demo
```
