...
This keeps the library dependency-free and is the Vue analog of the React port's Context + hooks pattern. (The optional starter template uses Pinia for app-level auth state, but that lives outside this package.)
...
**i18n** is not built into the core component library — its strings come from the props and menu data you pass in, so you localize at the app level with your own solution (for example `@nuxtjs/i18n`). The optional `adminlte-starter` template demonstrates a full i18n setup (en + es with a language switcher).
