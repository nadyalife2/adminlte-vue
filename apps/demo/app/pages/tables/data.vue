...
// Mirrors the original Data Tables demo: a Tabulator grid with header
// filters, an external filter input, and CSV/JSON/print export buttons.
const rows = [
...
onMounted(async () => {
  ...
  table = new TabulatorFull(tableEl, {
    data: rows,
    layout: 'fitColumns',
    pagination: true,
    paginationSize: 10,
  ...
  const wire = (id: string, type: string, handler: (e: Event) => void) => {
  ...
  wire('export-csv', 'click', () => table.download('csv', 'users.csv'))
  wire('export-json', 'click', () => table.download('json', 'users.json'))
  wire('print-table', 'click', () => table.print(false, true))
})
...
<template>
  <LteAppContent
    title="Data Tables"
    :breadcrumbs="[{ label: 'Home', href: '#' }, { label: 'Tables', href: '#' }, { label: 'Data' }]"
  >
            <div class="card">
