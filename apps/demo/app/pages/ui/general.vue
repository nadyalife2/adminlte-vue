...
onMounted(async () => {
  ...
  document.querySelectorAll('[data-bs-toggle="tooltip"]').forEach((el) => {
    tooltips.push(new bootstrap.Tooltip(el as HTMLElement))
  })
  document.querySelectorAll('[data-bs-toggle="toast"]').forEach((btn) => {
    const handler = (event: Event) => {
      event.preventDefault()
      const toastEl = document.getElementById(btn.getAttribute('data-bs-target') || '')
...
<template>
  <LteAppContent
    ...
    :breadcrumbs="[{ label: 'Home', href: '#' }, { label: 'General UI Elements' }]"
  >
            <div class="row g-4">
              <!--begin::Col-->
              <div class="col-12">
